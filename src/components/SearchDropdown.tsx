...
const SearchDropdown = ({ onClose, onSearch }: SearchDropdownProps) => {
  ...
  const popular = [
    { rank: 1, term: '엔진11' }, { rank: 2, term: 'RTX 4060' },
    { rank: 3, term: '픽시' }, { rank: 4, term: '아이폰 15' },
    { rank: 5, term: '패딩' }, { rank: 6, term: '에어팟' }
  ];
    ...
    <div className="absolute top-[calc(100%+1px)] left-0 w-full bg-white border border-gray-200 shadow-xl z-50">
      ...
      <div className="p-5 min-h-[300px]">
        {activeTab === 'recent' ? (
          ...
          <ul className="space-y-4">
            {popular.map((item) => (
              <li key={item.rank} onClick={() => onSearch(item.term)} className="flex items-center gap-4 cursor-pointer">
                <span className={`text-sm font-bold w-4 ${item.rank <= 3 ? 'text-[#ff5058]' : 'text-gray-800'}`}>{item.rank}</span>
      ...
      <div className="bg-[#f9f9f9] px-4 py-2 flex justify-between items-center border-t border-gray-100">
        <button onClick={handleDeleteAll} className="text-xs text-gray-400 hover:text-gray-600 flex items-center gap-1">
          🗑 검색어 전체삭제
        </button>
        <button onClick={onClose} className="text-xs text-gray-400 font-bold hover:underline">닫기</button>
      </div>
    </div>
  );
};
